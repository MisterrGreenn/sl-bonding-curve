...
describe('bonding_curve', () => {
  ...
  it('add liquidity', async () => {
    try {
      const [poolPda] = PublicKey.findProgramAddressSync([Buffer.from(POOL_SEED_PREFIX), mint1.toBuffer()], program.programId);
      const [liquidityProviderAccount] = PublicKey.findProgramAddressSync([Buffer.from(LIQUIDITY_SEED), poolPda.toBuffer(), user.publicKey.toBuffer()], program.programId);

      const [poolSolVault] = PublicKey.findProgramAddressSync([Buffer.from(SOL_VAULT_PREFIX), mint1.toBuffer()], program.programId);
      const poolToken = await getAssociatedTokenAddress(mint1, poolPda, true);
  ...
  it('Sell token', async () => {
    try {
      ...
      const poolToken = await getAssociatedTokenAddress(mint1, poolPda, true);
      const userAta1 = await getAssociatedTokenAddress(mint1, user.publicKey);
      const [poolSolVault, bump] = PublicKey.findProgramAddressSync([Buffer.from(SOL_VAULT_PREFIX), mint1.toBuffer()], program.programId);
      const tx = new Transaction().add(
        ComputeBudgetProgram.setComputeUnitLimit({ units: 200_000 }),
        ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200_000 }),
        await program.methods
